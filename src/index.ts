...
function getApiKey(): string {
  ...
  if (!key) {
    throw new Error(
      "未设置 BLOCKBEATS_API_KEY 环境变量，请先申请：https://www.theblockbeats.info/"
    );
  }
  return key;
}
...
const server = new Server(
  { name: "blockbeats-mcp", version: "1.0.0" },
  { capabilities: { tools: {} } }
);
  ...
  try {
    ...
    switch (name) {
      ...
      case "get_us_treasury_yield":
        data = await fetchApi("/v1/data/us10y", { type: (a.timeframe as string) || "1D" });
        break;
      case "get_dxy_index":
        data = await fetchApi("/v1/data/dxy", { type: (a.timeframe as string) || "1D" });
        break;
      case "get_m2_supply":
        data = await fetchApi("/v1/data/m2_supply", { type: (a.timeframe as string) || "1Y" });
        break;
      ...
      case "get_contract_oi_data":
        data = await fetchApi("/v1/data/contract", { dataType: (a.dataType as string) || "1D" });
        break;
